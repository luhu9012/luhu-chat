...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest LuhuChat image. Here are the commands required to perform these steps:

<Steps>
### Stop and Remove the Current Running LuhuChat Container

Assuming the LuhuChat container is named `lobe-chat`, use the following commands to stop and remove the currently running LuhuChat container:
...
### Pull the Latest LuhuChat Image

Use the following command to pull the latest Docker image for LuhuChat:
...
Redeploy the LuhuChat container using the newly pulled image:
...
No need to worry, you won't. All of LuhuChat's chat records are stored in your local browser. Therefore, when redeploying LuhuChat using Docker, your chat records will not be lost.
