...
We provide a [Docker image][docker-release-link] for you to deploy the LuhuChat service on your private device.
...
Use the following command to start the LuhuChat service with one click:
...
- For the complete list of environment variables supported by LuhuChat, please refer to the [Environment Variables](/zh/self-hosting/environment-ariable) section.
