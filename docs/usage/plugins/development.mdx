...
A LuhuChat plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in LuhuChat.
...
<<<<<<< HEAD:docs/usage/plugins/plugin-development.mdx
### **`2`** Add Local Plugin in LuhuChat Role Settings

Next, go to LuhuChat, create a new assistant, and go to its session settings page:
=======
...
>>>>>>> 0b1d4cdbdee0241b3965a4c86ef4bcddb0c3fbd9:docs/usage/plugins/development.mdx
...
<<<<<<< HEAD:docs/usage/plugins/plugin-development.mdx
After completing these operations, you have understood the basic process of adding custom plugins and using them in LuhuChat.
=======
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address LuhuChat loads the plugin's front-end interface. Since LuhuChat plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for LuhuChat to query the plugin's API interface. LuhuChat's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, LuhuChat will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for LuhuChat is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

<<<<<<< HEAD:docs/usage/plugins/plugin-development.mdx
For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest (http://localhost:3400/api/gateway), LuhuChat> will directly request this address, and then only the corresponding gateway needs to be created at that address.
=======
...
```ts
import { createLuhuChatPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createLuhuChatPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in LuhuChat [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing LuhuChat to access the local plugin service.
...
LuhuChat implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and LuhuChat. You can actively retrieve the current message data from LuhuChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from LuhuChat
